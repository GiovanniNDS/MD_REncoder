...
MD_REncoder::MD_REncoder(PinName pinA, PinName pinB):
...
{
  DigitalIn(_pinA, PullUp);
  DigitalIn(_pinB, PullUp);
...
{
  uint8_t pinstate = (DigitalIn(_pinB).read() << 1) | DigitalIn(_pinA).read();
